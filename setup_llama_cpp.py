...
# Parse number of parallel jobs from command-line
...
# Build with -j X
try:
    subprocess.run(["cmake", "--build", "build", "--config", "Release", f"-j{args.jobs}"], cwd=repo_dir, check=True)
    print(f"Build completed with -j{args.jobs}.")
except subprocess.CalledProcessError as e:
